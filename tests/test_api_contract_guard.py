...
def test_nullable_to_non_nullable_is_narrowing(write):
...
def test_openapi_31_type_list_null_is_treated_as_nullable(write):
    base = doc({"/a": {"get": {"responses": resp(props={"cursor": {"type": ["string", "null"]}})}}})
    cand = doc({"/a": {"get": {"responses": resp(props={"cursor": {"type": "string"}})}}})
    report = run(write, base, cand)
    assert "AC005" in codes(report)
...
def test_a_renamed_parameter_is_detected_and_suppresses_ac002(write):
    ...
    assert "AC002" not in codes(report)  # the rename is not a brand-new required field
